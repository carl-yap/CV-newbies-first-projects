...
# NOTE: `with` keyword can be used for one-time use file processing
...
while True:
    ...
    qr_info = decode(frame) # returns Decoded list

    if len(qr_info) > 0: # if a QR code is detected

        qr = qr_info[0] # limited to one QR at a time
        ...
        frame = cv2.polylines(frame, [np.array(polygon)], True, (255, 0, 0), 5) # outlining the Decoded's polygon
